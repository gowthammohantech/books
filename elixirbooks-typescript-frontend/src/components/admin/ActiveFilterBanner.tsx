...
const ActiveFilterBanner: React.FC<Props> = ({ filters, onClear }) => {
    ...
    return (
        <div className="mb-3 flex flex-wrap items-center gap-2 rounded-md border border-accent bg-accent px-3 py-2 text-sm">
            <span className="font-medium text-primary">Filtered from report:</span>
            {filters.map((f) => (
                <span
                    key={f.label}
            ...
            <button
                type="button"
                onClick={onClear}
                className="ml-auto inline-flex items-center gap-1 text-xs font-medium text-primary hover:text-primary cursor-pointer"
            >
