...
const EditCreditNote: React.FC = () => {
    ...
    useEffect(() => {
        const fetchCreditNoteDetails = async () => {
            try {
                setIsFetching(true);
                const response = await axios.get(`${Constants.FETCH_CREDIT_NOTE_FOR_EDIT_URL}/${id}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                ...
                if (creditNoteDetail) {
                    ...
                    setDocCreatedAt((creditNoteDetail.createdAt as string | undefined) ?? null);
                    setInvoiceFormData((prev) => ({
                        ...prev,
                        invoiceId: creditNoteDetail.invoice.id,
                        status: creditNoteDetail.status,
                        creditNoteDate: creditNoteDetail.creditNoteDate ? new Date(creditNoteDetail.creditNoteDate) : null,
                        billFrom: creditNoteDetail?.billFrom?.id,
                        billTo: creditNoteDetail?.billTo?.id ?? '',
                        items: hydrateLineCentres(creditNoteDetail.items, creditNoteDetail.costCenterId),
                        notes: creditNoteDetail.notes,
                        termsAndCondition: creditNoteDetail.termsAndCondition,
                        costCenterId: creditNoteDetail.costCenterId ?? '',
                        bank: creditNoteDetail.bank?.id,
                        sign_type: creditNoteDetail.sign_type ?? 'none',
                        signatureId: creditNoteDetail.signature?.id,
                        signatureName: creditNoteDetail.signature?.name,
                        esignDataUrl: creditNoteDetail.signature?.image,
                        subTotal: creditNoteDetail.taxableAmount,
                        totalTax: creditNoteDetail.vat,
                    ...
                    if (creditNoteDetail.billTo) {
                        let _customer = {
                            ...
                            phone: creditNoteDetail.billTo.phone,
                            image: creditNoteDetail.billTo.image
                        };
            ...
            try {
                ...
                if (response.data.data.length > 0) {
                    ...
                    availableInvoices.map((item: any) => {
                        setInvoiceOptions((prevOptions) => [...prevOptions, { id: item.id, name: item.invoiceNumber }]);
                    })
                }
            } catch (error) {
    ...
    const validateCreditNoteData = () => {
        ...
        if (!invoiceFormData.invoiceId) newErrors.invoiceId = 'Invoice is required.'
        //order date required
        if (!invoiceFormData.creditNoteDate) newErrors.creditNoteDate = 'Credit note date is required.';
        //status required
    ...
    const saveCreditNote = async (e: React.FormEvent) => {
        ...
        try {
            setIsSaving(true);
            await axios.put(`${Constants.UPDATE_CREDIT_NOTE_URL}/${id}`, formData, {
                headers: {
                    ...
                    <div className="w-full">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 w-full">
                            <div className='w-full flex flex-col justify-end'>
                                <label htmlFor="" className="block text-sm font-medium text-gray-700 ">Invoice <em className="text-destructive">*</em></label>
                                <SearchableDropdown
                                    options={invoiceOptions}
                                    placeholder='Select Invoice'
                                    value={invoiceOptions.find((option) => option.id === invoiceFormData.invoiceId) || null}
                                    inputValue={invoiceSearchInput}
                                    onInputChange={(_, value) => setInvoiceSearchInput(value)}
                                    onChange={(_, value) => {
                                        if (value && (value as OptionType).id !== invoiceFormData.invoiceId) {
                                            handleInvoiceChange(value as OptionType);
                                            setInvoiceSearchInput('');
                                        }
                                    }}
                                />
                                {formErrors?.invoiceId && <span className="text-destructive text-sm">{formErrors.invoiceId}</span>}
                            </div>
                            <div className="w-full">
                                <DateInput
                                    label="Credit Note Date"
                                    value={invoiceFormData.creditNoteDate}
                                    onChange={(newDate) => handleFormChange('creditNoteDate', newDate)}
                                    isRequired
                                />
                                {formErrors?.creditNoteDate && <span className="text-destructive text-sm">{formErrors.creditNoteDate}</span>}
                            </div>
                            <div className="w-full">
                                <label className="block text-sm font-medium text-gray-700 ">
                                    Status <em className='text-destructive'>*</em>
                                </label>
                                <SearchableDropdown
                                ...
                                />
                                {formErrors?.status && <span className="text-destructive text-sm">{formErrors.status}</span>}
                            </div>
                    ...
                    <div>
                        ...
                        {activeInfoTab === 'notes' && (
                            <div>
                                ...
                                <textarea value={invoiceFormData.notes} onChange={(e) => handleFormChange('notes', e.target.value)} rows={4} placeholder="Enter Notes" className="border border-gray-300 rounded-md px-4 py-2 w-full  text-gray-950  focus:outline-none focus:ring-1 focus:ring-ring"></textarea>
                            </div>
                        )}
                        {activeInfoTab === 'termsAndCondition' && (
                            <div>
                                <label className="block text-sm font-medium text-gray-700 ">Terms & Conditions</label>
                                <textarea value={invoiceFormData.termsAndCondition} onChange={(e) => handleFormChange('termsAndCondition', e.target.value)} rows={4} placeholder="Enter Terms & Conditions" className="border border-gray-300 rounded-md px-4 py-2 w-full  text-gray-950  focus:outline-none focus:ring-1 focus:ring-ring"></textarea>
                            </div>
                        )}
                        {activeInfoTab === 'bank' && (
                            <div>
                    ...
                    <div className="bg-white  p-4 rounded-lg border border-gray-200  space-y-3">
                        ...
                        {invoiceFormData.sign_type !== 'none' && (invoiceFormData.sign_type === 'digitalSignature' ? (
                            ...
                            <div>
                                ...
                                <input name='signatureName' type="text" value={invoiceFormData.signatureName} onChange={e => handleFormChange('signatureName', e.target.value)} placeholder="Enter Signature Name" className="border border-gray-300 rounded-md px-4 py-2 w-full  text-gray-950  focus:outline-none focus:ring-1 focus:ring-ring" />
