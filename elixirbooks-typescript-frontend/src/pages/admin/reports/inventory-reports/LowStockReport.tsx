...
const LowStockReport: React.FC = () => {
    ...
    const [lowStockReport, setLowStockReport] = useState<LowStockReportShape[]>([]);
    ...
    const fetchLowStockReport = async (range = dateRange) => {
        try {
            setIsLoading(true);
            ...
            if (response.data.data) {
                setLowStockReport(response.data.records);
    ...
    const clearAllFilters = () => {
        ...
        setSearchInput('');
        fetchLowStockReport({ startDate: null, endDate: null });
    }
    ...
    return (
        <div className="space-y-4">
            ...
            <Table headers={['#', 'Name', 'Item Type', 'Category', 'Unit', 'Purchase Price', 'Selling Price', 'Current Stock']}>
                {!isLoading && lowStockReport && lowStockReport.map((item: LowStockReportShape, index: number) => (
                    <TableRow
                        key={item.id}
                        row={item}
                        index={index + 1}
