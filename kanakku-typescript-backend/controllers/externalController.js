...
// - ssoExchange:  verify a whatsappcrm-issued HMAC JWT, upsert a kanakku User
//                 by email, mint a kanakku session token. Called by the
//                 kanakku frontend's /sso landing route.
...
// the source of truth for contacts. Kanakku stays standalone for everything
// else.
...
exports.ssoExchange = async (req, res) => {
  ...
  if (!secret) {
    return res.status(503).json({
      success: false,
      message: 'SSO not configured on this kanakku instance.',
    });
  }
  ...
  try {
    ...
    if (!user) {
      // First-time SSO from whatsappcrm — provision a kanakku user.
      ...
      // password through kanakku's normal flow.
...
const resolveOwnerUserId = async () => {
  // V1 design: a single admin owns externally-synced customers. Kanakku's
...
exports.upsertCustomer = async (req, res) => {
  ...
  if (!ownerId) {
    return res.status(409).json({
      success: false,
      message: 'No admin user configured on kanakku yet — register an admin first.',
    });
  }

  // Kanakku Customer requires a unique email per user. whatsappcrm contacts
  ...
  try {
    ...
    // duplicate-key path: a kanakku-native customer already owns this email.
    // Bail with a 409 so whatsappcrm logs it without retrying forever.
    if (err && err.code === 'P2002') {
      return res.status(409).json({
        success: false,
        message: 'A kanakku customer with this email already exists outside the integration.',
