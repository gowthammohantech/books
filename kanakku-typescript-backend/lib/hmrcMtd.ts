...
const VENDOR_PRODUCT_NAME = 'Kanakku';
...
export interface FraudContext {
  ...
  deviceId?: string;
  /** The originating end-user id within Kanakku — Gov-Client-User-IDs. */
  userId?: string;
...
export function fraudHeaders(ctx: FraudContext): Record<string, string> {
  ...
  if (ctx.userId) headers['Gov-Client-User-IDs'] = `kanakku=${encodeURIComponent(ctx.userId)}`;
