...
const AddCreditNote: React.FC = () => {
    ...
    // Apply document defaults once loaded — only when no parent invoice has been selected
    // (handleInvoiceChange already sets currencyCode/sign_type from the parent invoice)
    useEffect(() => {
    ...
    // When opened from an invoice (Credit Note action), prefill from that invoice.
    useEffect(() => {
        const invoiceIdParam = searchParams.get('invoiceId');
        if (invoiceIdParam && token) {
            handleInvoiceChange({ id: invoiceIdParam, name: '' } as OptionType);
        }
    ...
    const handleInvoiceSelect = (invoice: OptionType) => {
        if (invoice) {
            ...
            setInvoiceFormData(prev => ({ ...prev, invoiceId: '' }));
        }
    }
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 w-full">
                            <div className='w-full flex flex-col justify-end'>
                                ...
                                <SmartDropdown
                                    ...
                                    onSelect={(invoice) => handleInvoiceSelect(invoice as OptionType)}
