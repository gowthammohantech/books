    ...
    return (
        <label className={`inline-flex items-center cursor-pointer ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}>
            ...
            <div className="relative w-11 h-6 bg-gray-200 peer-checked:bg-primary rounded-full peer-focus:ring-2 peer-focus:ring-ring transition-all duration-300">
                <div
                    className={`absolute top-0.5 left-1 w-5 h-5 bg-surface-white rounded-full shadow-md transform transition-transform duration-300 ${checked ? 'translate-x-full' : ''
                        }`}
                ></div>
            </div>
        </label>
