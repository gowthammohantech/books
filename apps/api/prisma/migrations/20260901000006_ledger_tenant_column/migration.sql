-- P2 / M7: denormalize `tenantId` onto the general ledger lines.
--
...
-- JournalLine: tenant via JournalLine.journalEntryId -> JournalEntry."userId"
ALTER TABLE "JournalLine" ADD COLUMN "tenantId" TEXT;

UPDATE "JournalLine" c
SET "tenantId" = p."userId"
FROM "JournalEntry" p
WHERE p.id = c."journalEntryId";
...
DELETE FROM "JournalLine" WHERE "tenantId" IS NULL;
