        ...
        <Modal isOpen title="Void Payment" onClose={onClose} size="sm">
            <div className="space-y-4">
                <p className="text-sm text-gray-700">
                    ...
                    <strong>{formatCurrency(payment.paidAmount)}</strong> recorded on{' '}
...
const PurchasePaymentHistoryPanel: React.FC<PurchasePaymentHistoryPanelProps> = ({ purchaseId, onChanged }) => {
    ...
    const handleVoidConfirm = async (voidReason: string) => {
        ...
        try {
            ...
            toast.error(axiosErr?.response?.data?.error || 'Failed to void payment.');
        }
    };
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {payments.map((p) => (
                                <tr key={p.id} className={p.isVoided ? 'bg-gray-50 text-gray-400' : ''}>
                                    {/* Date */}
                                    <td className="px-3 py-2 whitespace-nowrap">{formatDate(p.paymentDate)}</td>

                                    {/* Amount */}
                                    <td className="px-3 py-2 whitespace-nowrap font-medium">
                                        {p.isVoided ? (
                                            <span className="line-through">{formatCurrency(p.paidAmount)}</span>
                                        ) : (
                                    ...
                                    {/* Recorded by */}
                                    <td className="px-3 py-2 whitespace-nowrap">{fullName(p.createdByUser)}</td>
...
export default PurchasePaymentHistoryPanel;
