 ...
 * PettyCash with `{ isDeleted: false }` only, no `tenantId`. PettyCash has a
 * `tenantId` column (Task 2, migration 20260702000001_pettycash_tenant_scope)
 * and every read must be a strict `{ tenantId, ... }` match — no OR-null
