...
export async function importGeoDataset(): Promise<void> {
  ...
  try {
    ...
    //    Build sourceCountryId → elixirBooksCountryId map for state FK resolution.
    // -----------------------------------------------------------------------
    let countriesInserted = 0;
    let countriesUpdated = 0;
    const sourceIdToElixirBooksId = new Map<number, string>();
    ...
    for (const src of sourceCountries) {
      ...
      let elixirBooksId: string;

      if (existingId) {
        // Keep existing id (preserves c-india, c-united-states, etc.)
        await prisma.country.update({ where: { id: existingId }, data: fields });
        elixirBooksId = existingId;
        countriesUpdated++;
      } else {
        elixirBooksId = randomUUID();
        await prisma.country.create({ data: { id: elixirBooksId, ...fields } });
        countriesInserted++;
        // Register new iso2 so subsequent source rows resolve it
        if (iso2Upper) existingCountryByIso2.set(iso2Upper, elixirBooksId);
      }

      sourceIdToElixirBooksId.set(src.id, elixirBooksId);
    }
    ...
    for (const src of sourceCountries) {
      const elixirBooksCountryId = sourceIdToElixirBooksId.get(src.id);
      if (!elixirBooksCountryId) continue; // country was skipped (no name)
