...
| CPU | 2 vCPU |
| RAM | 4 GB |
| Disk | 20 GB, plus whatever uploads need |
| OS | Linux, any modern distro |
| Docker Engine | 24+ |
| Docker Compose plugin | v2 (`docker compose`, not the legacy `docker-compose`) |
| PostgreSQL | 16 — provided by the compose stack, no separate install |
| Node.js | 20 — runs inside the API container, no host install |
| Ports | 80 and 443 reachable, for web + TLS |

Redis is not required. It is an opt-in service, described in
